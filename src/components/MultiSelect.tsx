...
const MultiSelectFooter = ({ size = "sm", onReset, onSelectAll, className }: MultiSelectFooterProps) => {
    ...
    return (
        <div className={cx("flex items-center justify-between border-t border-[var(--border-subtle)] p-3", className)}>
            <Button size={btnSize} variant="secondary" onClick={onReset}>
                Reset
            </Button>
            <Button size={btnSize} variant="secondary" onClick={onSelectAll}>
                Select all
            </Button>
        </div>
    );
            ...
            <div className={cx("flex flex-col gap-1.5", className)}>
                ...
                <AriaDialogTrigger>
                    <AriaButton
                        ...
                        className={(state) =>
                            cx(
                                ...
                                (state.isFocusVisible || state.isPressed) && "ring-2 ring-[var(--graffiti-500)]",
                        ...
                        >
                            {hasSelection ? (
                                <span className={cx("flex items-center", sizes[size].textContainer)}>
                                    <span className={cx("font-medium text-[var(--text-primary)]", sizes[size].text)}>
                                        {selectedCountFormatter ? selectedCountFormatter(selectedCount) : `${selectedCount} selected`}
                                    </span>
                                    {supportingText && <span className={cx("text-[var(--text-tertiary)]", sizes[size].text)}>{supportingText}</span>}
                                </span>
                            ) : (
                                <span className={cx("text-[var(--text-[var(--text-tertiary)])]", sizes[size].text)}>{placeholder}</span>
                            )}
                    ...
                    <AriaPopover
                        ...
                        className={(state) =>
                            cx(
                                "w-(--trigger-width) origin-(--trigger-anchor-point) overflow-hidden rounded-lg bg-[var(--bg-surface)] shadow-[var(--shadow-lg)] ring-1 ring-[var(--border-subtle)] outline-hidden will-change-transform",
                                state.isEntering &&
                    ...
                    >
                        <AriaDialog className="outline-hidden">
                            <AriaAutocomplete filter={contains} inputValue={searchValue} onInputChange={setSearchValue}>
                                {showSearch && (
                                    <div className={cx("border-b border-[var(--border-subtle)]", searchSizes[size].wrapper)}>
                                        <AriaSearchField aria-label="Search" value={searchValue} onChange={setSearchValue} autoFocus>
                                            <div className={cx("flex items-center", searchSizes[size].root)}>
                                                <SearchLg data-icon aria-hidden="true" className="shrink-0 text-[var(--icon-disabled)]" />
                                                <AriaInput
                                                    placeholder="Search"
                                                    className={cx(
                                                        "w-full appearance-none bg-transparent text-[var(--text-primary)] caret-alpha-black/90 outline-hidden placeholder:text-[var(--text-[var(--text-tertiary)])]",
