...
export const RadioButtonBase = ({ className, isFocusVisible, isSelected, isDisabled, size = "sm" }: RadioButtonBaseProps) => {
    return (
        <div
            className={cx(
                "flex size-4 shrink-0 cursor-pointer appearance-none items-center justify-center rounded-full bg-[var(--bg-surface)] ring-1 ring-[var(--border-default)] ring-inset",
                size === "md" && "size-5",
                isSelected && "bg-[var(--bg-brand-default)] ring-[var(--graffiti-500)]",
                isDisabled && "cursor-not-allowed opacity-50",
                isDisabled && !isSelected && "bg-[var(--bg-muted)]",
