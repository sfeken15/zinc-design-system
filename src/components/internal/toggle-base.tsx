...
export const ToggleBase = ({ className, isHovered, isDisabled, isFocusVisible, isSelected, slim, size = "sm" }: ToggleBaseProps) => {
    ...
    return (
        <div
            className={cx(
                "cursor-pointer rounded-full bg-[var(--bg-muted)] ring-[0.5px] ring-[var(--border-subtle)] outline-[var(--graffiti-500)] transition duration-150 ease-linear ring-inset",
                isSelected && "bg-[var(--bg-brand-default)]",
                isSelected && isHovered && "bg-[var(--bg-brand-hover)]",
        ...
        >
            <div
                ...
                className={cx(
                    "rounded-full bg-[var(--text-inverse)] shadow-[var(--shadow-sm)]",

                    slim && "shadow-[var(--shadow-xs)]",
                ...
                <>
                    ...
                    {(label || hint) && (
                        <div className={cx("flex flex-col", styles[size].textWrapper)}>
                            {label && <p className={cx("text-[var(--text-secondary)] select-none", styles[size].label)}>{label}</p>}
                            {hint && (
                                <span className={cx("text-[var(--text-tertiary)]", styles[size].hint)} onClick={(event) => event.stopPropagation()}>
                                    {hint}
                                </span>
                            )}
                        </div>
