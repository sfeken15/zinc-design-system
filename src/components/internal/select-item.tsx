...
import type { SelectItemType } from "@/components/internal/select-shared";
import { SelectContext } from "@/components/internal/select-shared";
            ...
            {(state) => (
                <div
                    className={cx(
                        ...
                        (state.isFocused || state.isHovered || (state.isSelected && selectionIndicator !== "checkbox")) && "bg-[var(--bg-surface-raised)]",
                        state.isDisabled && "cursor-not-allowed opacity-50",
                        state.isFocusVisible && "ring-2 ring-[var(--graffiti-500)] ring-inset",
                ...
                >
                    ...
                    <div className={cx("flex w-full min-w-0 flex-1 flex-wrap", sizes[size].textContainer)}>
                        <AriaText slot="label" className={cx("truncate font-medium whitespace-nowrap text-[var(--text-primary)]", sizes[size].text)}>
                            {label || (typeof children === "function" ? children(state) : children)}
                        </AriaText>

                        {supportingText && (
                            <AriaText slot="description" className={cx("whitespace-nowrap text-[var(--text-tertiary)]", sizes[size].text)}>
                                {supportingText}
                            </AriaText>
                        )}
                    </div>
                    ...
                    {state.isSelected && selectionIndicator === "checkmark" && (
                        <Check aria-hidden="true" className={cx("ml-auto text-[var(--icon-brand)]", sizes[size].check)} />
                    )}
