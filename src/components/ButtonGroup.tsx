...
export const styles = sortCx({
    common: {
        root: [
            "group/button-group inline-flex h-max cursor-pointer items-center bg-[var(--bg-surface)] font-semibold whitespace-nowrap text-[var(--text-secondary)] shadow-skeuomorphic ring-1 ring-[var(--border-default)] outline-[var(--graffiti-500)] transition duration-100 ease-linear ring-inset",
            // Hover and focus styles
            "hover:bg-[var(--bg-surface-raised)] hover:text-[var(--text-[var(--text-secondary)])] focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-2",
            // Disabled styles
            "disabled:cursor-not-allowed disabled:text-[var(--text-secondary)]/50 disabled:*:opacity-50",
            // Selected styles
            "selected:bg-[var(--bg-surface-raised)] selected:text-[var(--text-[var(--text-secondary)])]",
        ].join(" "),
        icon: "pointer-events-none text-[var(--icon-disabled)] transition-[inherit] group-hover/button-group:text-[var(--icon-subtle)] group-selected/button-group:text-[var(--icon-subtle)]",
    },
    ...
    return (
        <ButtonGroupContext.Provider value={{ size }}>
            <AriaToggleButtonGroup
                ...
                className={cx("relative z-0 inline-flex w-max -space-x-px rounded-lg shadow-[var(--shadow-xs)]", className)}
                {...otherProps}
            >
                {children}
            </AriaToggleButtonGroup>
