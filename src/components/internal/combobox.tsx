    ...
    return (
        <AriaGroup
            ...
            className={({ isFocusWithin, isDisabled }) =>
                cx(
                    "relative flex w-full items-center gap-2 rounded-lg bg-[var(--bg-surface)] shadow-[var(--shadow-xs)] ring-1 ring-[var(--border-default)] outline-hidden transition-shadow duration-100 ease-linear ring-inset",
                    isDisabled && "cursor-not-allowed opacity-50",
                    isFocusWithin && "ring-2 ring-[var(--graffiti-500)]",
        ...
        >
            ...
            <div className="relative flex w-full items-center">
                {inputValue && (
                    <span className={cx("absolute top-1/2 z-0 inline-flex w-full -translate-y-1/2 truncate", sizes[size].textContainer)} aria-hidden="true">
                        <p className={cx("font-medium text-[var(--text-primary)]", sizes[size].text)}>{first}</p>
                        {last && <p className={cx("-ml-0.75 text-[var(--text-tertiary)]", sizes[size].text)}>{last}</p>}
                    </span>
                )}
                ...
                <AriaInput
                    ...
                    className={cx(
                        "z-10 w-full appearance-none bg-transparent text-transparent caret-alpha-black/90 placeholder:text-[var(--text-[var(--text-tertiary)])] focus:outline-hidden disabled:cursor-not-allowed",
                        sizes[size].text,
                    )}
                />
            </div>
            ...
            {shortcut && (
                <div
                    className={cx(
                        "absolute inset-y-0.5 right-0.5 z-10 hidden items-center rounded-r-[inherit] bg-linear-to-r from-transparent to-bg-[var(--bg-surface)] to-40% pl-8 md:flex",
                        sizes[size].shortcut,
                        shortcutClassName,
                    )}
                >
