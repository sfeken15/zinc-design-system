...
import { RadioButtonBase } from "@/components/RadioButton";
import { ToggleBase } from "@/components/internal/toggle-base";
        ...
        (state: MenuItemRenderProps & { className?: string }) => {
            if (selectionIndicator === "checkmark") {
                return (
                    <Check
                        aria-hidden="true"
                        className={cx("size-4 shrink-0 stroke-[2.25px] text-[var(--icon-brand)]", !state.isSelected && "invisible", state.className)}
                    />
                );
            }
    ...
    return (
        ...
        >
            {(state) => (
                <div
                    className={cx(
                        "relative flex items-center rounded-md px-2.5 py-2 outline-[var(--graffiti-500)] transition duration-100 ease-linear",
                        !state.isDisabled && "group-hover:bg-[var(--bg-surface-raised)]",
                        state.isFocused && "bg-[var(--bg-surface-raised)]",
                ...
                >
                    ...
                    {Icon && <Icon aria-hidden="true" className="mr-2 size-4 shrink-0 stroke-[2.25px] text-[var(--icon-disabled)]" />}

                    <span className={cx("grow truncate text-sm font-semibold text-[var(--text-secondary)]", state.isFocused && "text-[var(--text-[var(--text-secondary)])]")}>
                        {label || (typeof children === "function" ? children(state) : children)}
                    </span>

                    {addon && <span className="ml-1 shrink-0 pr-1 text-xs font-medium text-[var(--text-disabled)]">{addon}</span>}

                    {state.selectionMode !== "none" && (avatarUrl || Icon) && <SelectionIndicator {...state} className="ml-1" />}

                    {state.hasSubmenu && <ChevronRight aria-hidden="true" className="ml-auto size-4 shrink-0 stroke-[2.25px] text-[var(--icon-disabled)]" />}
                </div>
            )}
        </AriaMenuItem>
    );
...
const DropdownPopover = (props: DropdownPopoverProps) => {
    return (
        <AriaPopover
            ...
            className={(state) =>
                cx(
                    "w-62 origin-(--trigger-anchor-point) overflow-auto rounded-lg bg-[var(--bg-surface)] shadow-[var(--shadow-lg)] ring-1 ring-[var(--border-subtle)] will-change-transform",
                    state.isEntering &&
...
const DropdownSeparator = (props: AriaSeparatorProps) => {
    return <AriaSeparator {...props} className={cx("my-1 h-px w-full bg-border-[var(--border-subtle)]", props.className)} />;
};
...
const DropdownDotsButton = (props: AriaButtonProps & RefAttributes<HTMLButtonElement>) => {
    return (
        <AriaButton
            ...
            className={(state) =>
                cx(
                    "cursor-pointer rounded-md text-[var(--icon-disabled)] outline-[var(--graffiti-500)] transition duration-100 ease-linear",
                    (state.isPressed || state.isHovered) && "text-[var(--icon-subtle)]",
