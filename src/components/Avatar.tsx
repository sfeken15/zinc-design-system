...
import { AvatarOnlineIndicator, VerifiedTick } from "@/components/internal/avatar-base";
import { AvatarCount } from "@/components/internal/avatar-base/avatar-count";
...
export const Avatar = ({
    ...
    const renderMainContent = () => {
        ...
        if (initials) {
            return <span className={cx("text-[var(--text-disabled)]", styles[size].initials)}>{initials}</span>;
        }

        if (PlaceholderIcon) {
            return <PlaceholderIcon className={cx("text-[var(--icon-disabled)]", styles[size].icon)} />;
        }

        return placeholder || <User01 className={cx("text-[var(--icon-disabled)]", styles[size].icon)} />;
    };
    ...
    return (
        <div
            ...
            className={cx(
                ...
                // Focus styles
                focusable && "outline-transparent group-focus-visible:outline-2 group-focus-visible:outline-offset-2 group-focus-visible:outline-[var(--graffiti-500)]",
                border && "ring-1 ring-[var(--border-subtle)]",
        ...
        >
            <div
                className={cx(
                    "relative inline-flex size-full shrink-0 items-center justify-center overflow-hidden rounded-md bg-[var(--bg-muted)] outline-[0.5px] -outline-offset-[0.5px] outline-black/16 before:inset-[0.5px]",
                    rounded && "rounded-full",
                    canShowImage &&
                        size !== "xs" &&
